...
if ( ! function_exists( 'anymarket_get_loaded_wpemerge' ) ) {
	/**
	 * Get the currently loaded WP Emerge version, if any.
	 *
	 * @return array|false
	 */
	function anymarket_get_loaded_wpemerge() {
		/**
...
if ( ! function_exists( 'anymarket_should_load_wpemerge' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function anymarket_should_load_wpemerge( $name, $min, $max ) {
		$loaded = anymarket_get_loaded_wpemerge();
		...
		add_action(
			...
			function () use ( $name, $min, $max, $loaded ) {
				...
				$message = __( '%1$s requires WP Emerge version >= %2$s and < %3$s but %4$s has already loaded version %5$s.', 'anymarket' );
				?>
...
if ( ! function_exists( 'anymarket_declare_loaded_wpemerge' ) ) {
	/**
	 ...
	 * @return void
	 */
	function anymarket_declare_loaded_wpemerge( $name, $type, $file ) {
		$loaded = anymarket_get_loaded_wpemerge();
